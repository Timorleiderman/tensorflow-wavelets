...
class DWT(layers.Layer):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        print("inputs_padd shape", inputs_padd.shape)
        print("filter shape", self.db2_hpf.shape)
        a = tf.nn.conv2d(
            inputs_padd, self.db2_lpf, padding='VALID', strides=[1, 1, 1, 1],
        )
        print("lpf conv shape", a.shape)
        d = tf.nn.conv2d(
            inputs_padd, self.db2_hpf, padding='VALID', strides=[1, 1, 1, 1],
        )
        print("hpf conv shape", d.shape)
        a_ds = a[:, :, 1:inputs.shape[1]:2, :]
        d_ds = d[:, :, 1:inputs.shape[1]:2, :]
        print("a_ds  shape", a_ds.shape)
        ...
        print("a_ds_padd  shape", a_ds_padd.shape)
        print("d_ds_padd  shape", d_ds_padd.shape)
