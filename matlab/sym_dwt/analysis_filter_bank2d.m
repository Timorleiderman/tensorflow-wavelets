...
function [cA, cH, cV, cD] = analysis_filter_bank2d(x, LoD, HiD)
...
% low pass filter
lo_conv = conv2(x_extend, LoD(:),conv_type);
...
% high pass filter
hi_conv = conv2(x_extend, HiD(:),conv_type);
...
% low pass filter
lo_lo = conv2(lo_ext, LoD(:),conv_type);
lo_hi = conv2(lo_ext, HiD(:),conv_type);
hi_lo = conv2(hi_ext, LoD(:),conv_type);
hi_hi = conv2(hi_ext, HiD(:),conv_type);
