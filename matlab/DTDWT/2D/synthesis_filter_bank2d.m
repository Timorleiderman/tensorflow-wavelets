% synthesis filter 2d
function y = synthesis_filter_bank2d(cA, cD, LoR_row, HiR_row, LoR_col, HiR_col)
...
LL_us_conv = conv2(LL_us,LoR_col(:),conv_type);
%apply high pass filter
LH_us_conv = conv2(LH_us,HiR_col(:),conv_type);
% add approximation and details
LL_LH = LL_us_conv + LH_us_conv;
LL_LH = LL_LH(L+1:end-L-2,:);

LL_LH(1:L1-2, :) = LL_LH(1:L1-2, :) + LL_LH(2*w+[1:L1-2], :);
LL_LH = LL_LH(1:2*w, :);
% circular shift
n = 0:2*w-1;
n = mod(n-(1-L1/2), 2*w);
...
HH_us(1:2:end,:) =  HH;
%apply low pass filter
HL_us_conv = conv2(HL_us,LoR_col(:),conv_type);
%apply high pass filter
HH_us_conv = conv2(HH_us,HiR_col(:),conv_type);
% add details and details
HL_HH = HL_us_conv + HH_us_conv;
HL_HH = HL_HH(L+1:end-L-2,:);

HL_HH(1:L1-2, :) = HL_HH(1:L1-2, :) + HL_HH(2*w+[1:L1-2], :);
...
LL_LH_us_conv = conv2(LL_LH_us,LoR_row(:),conv_type);
HL_HH_us_conv = conv2(HL_HH_us,HiR_row(:),conv_type);
